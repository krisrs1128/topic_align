...
ggplot(key_counts %>% filter(m != "K10", alpha %in% subset_alpha)) +
  ...
  geom_point(aes(m, n_topics, size = n)) +
...
ggplot(perplexity) +
  geom_line(aes(K, perplexity, group = interaction(path, split), col = split), alpha = 0.4) +
  scale_y_log10() +
  facet_wrap(alpha ~ split, scales = "free_y")
#ggsave("~/Downloads/gradient-perplexity.png", dpi = 400)
```
