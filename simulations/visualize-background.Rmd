...
```{r}
figures[["paths"]] <- ggplot(key_counts %>% filter(m != "K10", alpha %in% subset_alpha)) +
...
library(patchwork)
figures[["paths"]] <- figures[["paths"]] + ggtitle("a") + theme(title = element_text(size = 14))
figures[["coherence"]] <- figures[["coherence"]] + ggtitle("b") +theme(title = element_text(size = 14))
figures[["refinement"]] <- figures[["refinement"]] + ggtitle("c") +theme(title = element_text(size = 14))
((figures[["paths"]] + plot_layout(guides = "keep")) / (figures[["coherence"]] / figures[["refinement"]] +
  plot_layout(guides = "collect"))) +
  plot_layout(heights = c(1, 2))
ggsave("~/Downloads/gradient-combined.png")
```
