...
ggplot(measures %>% filter(!(m %in% c("K1", "K10")))) +
  geom_split_violin(
    aes(m, refinement, fill = high_sim),
    bw = 0.04, size = 0.0,
  ) +
  scale_fill_manual(values = c("#913551", "#71aaa4")) +
  scale_y_log10() +
  labs(x = "K Topics", y = "refinement") +
  facet_grid( ~ method) +
  theme(legend.position = "none")
ggsave("~/Downloads/refinement_lda.png", dpi=500, width = unit(5, "cm"), height = unit(3, "cm"))
```
#### Gradient Experiment
...
```{r}
ggplot(measures %>% filter(alpha %in% subset_alpha)) +
  geom_split_violin(
    aes(m, refinement, fill = high_sim),
    bw = 0.04, size = 0
  ) +
  facet_grid(method ~ alpha) +
  scale_fill_manual(values = c("#913551", "#71aaa4")) +
  labs(y = "Refinement", x = "K Topics", fill = "Similarity > 0.95") +
  scale_y_log10() +
