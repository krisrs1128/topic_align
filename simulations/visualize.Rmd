...
ggplot(
  ...
  aes(m_ix, robustness, color = alpha, size = weight, alpha = weight)
  ) +
  ...
  geom_segment(aes(xend = m_next_ix, yend = robustness_next)) +
...
ggsave("~/Downloads/gradient_stability.png", dpi = 500, width = unit(8.5, "in"), height = unit(3, "in"))
```
...
ggplot(measures %>% filter(m != "K10")) +
  geom_point(
    aes(m, refinement, col = alpha),
    position = position_jitter(w = 0.1),
    alpha = 0.1, size = 0.2
  ) +
  scale_color_viridis_c() +
