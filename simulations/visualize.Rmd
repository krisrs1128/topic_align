...
read_and_process <- function(pattern, grouping, directory, max_K=10) {
  ...
  map_dfr(paths, read_csv) %>%
    mutate(
      ...
      method = "transport" # eventually include product too
    ) %>%
...
```{r}
stability_paths <- list.files("results/transport_lda_results", "*stability*", full = TRUE)
stability <- read_and_process("*stability*", c("id", "branch"), "results/transport_lda_results/")
ggplot(stability) +
...
```{r}
key_topics <- read_and_process("key", c("id"), "results/transport_lda_results")
ggplot(key_topics) +
