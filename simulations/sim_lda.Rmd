...
N <- 250
n_models <- 10
lambdas <- list(beta = 0.5, gamma = 0.5)
...
```{r}
alignment <- align_topics(lda_models, method = params$method, reg=10, top_n=3)
plot(alignment)
...
```{r}
write_csv(key_topics, file.path(parmas$out_dir, str_c("key_topics-", params$method, "-", params$id, ".csv")))
