...
```{r}
edges <- expand.grid(seq_along(fits), seq_along(fits))
gamma_hats <- map(fits, ~ .@gamma)
beta_hats <- map(fits, ~ t(.@beta))
alignment <- align_graph(edges, gamma_hats, beta_hats, gamma_similarities)
```

```{r, fig.height = 4.5, fig.width = 5.5}
G <- graph_data(alignment, gamma_hats)
ggraph(G, "stress") +
  geom_edge_link(aes(width = weight, alpha = weight)) +
  geom_node_label(aes(label = k_LDA, fill = m, size=mass)) +
...
gammas <- sim_data$gammas %>%
  split(.$environment) %>%
  map(~ select(., -environment)) %>%
  map(~ set_colnames(., seq_len(ncol(.))))
gammas <- c(gammas, gamma_hats)
names(gammas) <- seq_along(gammas)

betas <- rep(list(t(log(sim_data$B))), length(N))
betas <- c(betas, beta_hats)

edges <- expand.grid(
  seq_along(N),
  (length(N) + 1):length(gammas)
)

alignment <- align_graph(edges, gammas, betas, gamma_similarities)
G <- graph_data(alignment, gammas)
```

```{r, fig.height = 3, fig.width = 6}
G <- G %>%
  mutate(type = m %in% 1:6)
