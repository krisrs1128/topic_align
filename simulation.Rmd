...
```{r}
betas <- widen_betas(fits$betas)
masses <- gamma_masses(fits$gammas)
alignments <- transport_align(betas, masses)
head(alignments)
```
...
alignments %>%
  filter(w > 0.02) %>%
  ggplot(aes(x = k, y = k_LDA_, size = w, alpha = w)) +
  geom_point() +
  geom_segment(aes( xend = k_next, yend = k_LDA_next)) +
...
transport_align_pair(source, target)
```
