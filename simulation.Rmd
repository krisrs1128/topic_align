...
```{r}
simulate_replicate <- function(leaves, gamma) {
  x <- simulate_lda(leaves, gamma)
  fits <- run_lda_models(x, 2 ^ seq(0, n_lev), c(.1), "VEM", 123, reset = TRUE)
  alignment <- align_topics(fits)
  gamma <- endpoint_topics(fits$beta, alignment$gamma_alignment) %>%
    mutate(method = "gamma_alignment")
  beta <- endpoint_topics(fits$beta, alignment$beta_alignment) %>%
    mutate(method = "beta_alignment")
  bind_rows(gamma, beta)
}
  
edges <- map_dfr(seq_len(50), ~ simulate_replicate(leaves, gamma), .id = "replicate")
...
x <- simulate_lda(leaves %>% select(-node_id), gamma)
fits <- run_lda_models(x, unique_levels(betas), c(.1), "VEM", 123, reset = TRUE)
alignment <- align_topics(fits)
visualize_aligned_topics(alignment)
visualize_aligned_topics(alignment, method = "beta_alignment")
```
