...
and recovered topics^[We would have to identify reasonable choices of
...
beta_list <- function(tree_data, l = 2, prefix = "beta") {
  beta <- tree_data %>%
    ...
    select(matches("[0-9]+")) %>%
    t()
  colnames(beta) <- str_c(prefix, 1:ncol(beta))
  list(mass = rep(1 / ncol(beta), ncol(beta)), pos = beta)
}

sources <- beta_list(cbind(tree, topics), 2, "sources")
targets <- beta_list(cbind(tree, topics), 3, "targets")
...
```{r, fig.cap = "Topic alignments deteriorate when the amount of time given to evolve is reduced."}
tree <- generate_tree(n_lev, lambda = 100)
topics <- tree_topics(tree)
sources <- beta_list(cbind(tree, topics), 2, "sources")
targets <- beta_list(cbind(tree, topics), 3, "targets")
alignment <- transport_align_pair(sources, targets, lambda = 0.01)
