...
```
```{r, fig.cap = "Alignment when providing true topic betas and gammas. The only error here is in alignment, not topic estimation.", fig.width = 4, fig.height = 2.5}
fits$betas <- reshape_tree_beta(tree, topics)
fits$gammas <- agglomorate_gammas(tree, gamma)
alignment <- align_topics(fits)
head(alignment$gamma_alignment)
visualize_aligned_topics(alignment)
```
