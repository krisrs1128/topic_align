...
```{r}
tree <- generate_tree()
betas <- tree_topics(tree)
true_links <- make_links(tree, betas)

beta <- betas %>%
  filter(m == max(m)) %>%
  select(-m, -k)
gammas <- rdirichlet(100, rep(0.05, nrow(beta)))
x <- simulate_lda(beta, gammas)
...
edges <- endpoint_topics(beta_hats, wrapper$alignment) %>%
  mutate(replicate = 1)
...
edge_scores <- juice(edge_fits) %>%
  mutate(endpoints = str_c(k, k_next, sep = "_"))
