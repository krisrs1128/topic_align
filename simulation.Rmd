...
```
```{r, fig.cap = "Alignment when using estimated topics."}
alignment <- align_topics(x, fits)
visualize_aligned_topics(alignment)
```

```{r, fig.cap = "Alignment when providing true topic betas and gammas. The only error here is in alignment, not topic estimation."}
fits$betas <- reshape_tree_beta(tree, topics)
fits$gammas <- agglomorate_gammas(tree, gamma)
alignment <- align_topics(x, fits)
visualize_aligned_topics(alignment)
```
