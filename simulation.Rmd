...
```{r}
tree <- generate_tree(n_levels = 4)
betas <- tree_topics(tree, V = 50)
...
edges <- map_dfr(c("beta_alignment", "gamma_alignment"), ~ mutate(links, method = ., estimate = FALSE)) %>%
  bind_rows(simulate_replicates(leaves, gammas, 1) %>% mutate(estimate = TRUE))
...
edge_scores <- juice(edge_fits) %>%
  mutate(
    method_label = ifelse(method == "beta_alignment", "Beta", "Gamma"),
    estimate_label = ifelse(estimate, "Estimated Topic", "Topic")
    )
ggplot(edge_scores, aes(umap_1, umap_2, size = weight, col = estimate_label)) +
  geom_point(data = edge_scores %>% filter(!estimate), alpha = 1) +
  geom_point(data = edge_scores %>% filter(estimate), alpha = 0.4) +
  facet_grid(method_label ~ m) +
  scale_color_manual(values = c("#e97451", "#0c0c0c")) +
...
for (i in seq_len(n_reps)) {
  print(i)
  for (sim_method in c("dirichlet_multinomial", "topic_curve", "lda")) {
    if (sim_method == "dirichlet_multinomial") {
      x <- dirichlet_multinomial(N, V)
    } else if (sim_method == "topic_curve") {
      betas <- topic_curve(10, 3, V = V)
      gammas <- rdirichlet(N, alpha = rep(lambda, 10))
      x <- simulate_lda(betas, gammas)
    } else if (sim_method == "lda") {
      betas <- rdirichlet(K, rep(lambda, V))
      gammas <- rdirichlet(N, alpha = rep(lambda, K))
      x <- simulate_lda(betas, gammas)
    }
    
    scores[[j]] <- fit_wrapper(x, 8)$alignment %>%
      compute_score_along_branches() %>%
      mutate(
        simulation = sim_method,
        replicate = i
      )
    j <- j + 1
  }
...
scores <- bind_rows(scores, .id = "j")
```

```{r, fig.width = 6, fig.height = 6}
ggplot(scores, aes(m, score)) +
  geom_point(size = 0.5) +
  geom_line(
    aes(group = interaction(replicate, branch)),
    alpha = 0.8
  ) +
  facet_grid(replicate ~ simulation)
ggsave("~/Desktop/simulation_stability_reps.png", dpi = 800)
```

```{r, fig.width = 10, fig.height = 10}
for (i in seq_len(n_reps)) {
  x_star <- dirichlet_multinomial(1000, 50)
  p[[i]] <- vis_wrapper(x_star, 8)["p2"]
}

grid.arrange(grobs = p)
#ggsave("dirichlet_multinomial.png", dpi = 800)
```
...
for (i in seq_len(9)) {
  ...
  gammas <- rdirichlet(100, alpha = rep(0.1, 10))
  x <- simulate_lda(betas, gammas)
  p[[i]] <- vis_wrapper(x, 8)[["p2"]]
}
grid.arrange(grobs = p)
ggsave("topic_curve.png", dpi = 800)
```
...
p <- list()
for (i in seq_len(9)) {
  betas <- rdirichlet(4, rep(0.1, 50))
  gammas <- rdirichlet(100, alpha = rep(0.1, 4))
  x <- simulate_lda(betas, gammas)
  p[[i]] <- vis_wrapper(x, 8)[["p2"]]
...
ggsave("true_lda.png", dpi = 800)
result <- vis_wrapper(x, 8)
```
...
ggplot(weight_comparison, aes(gamma_alignment, beta_alignment)) +
  ...
  labs(x = expression(paste(gamma, "-alignment")), y = expression(paste(beta, "-alignment")), col = "M") +
  coord_fixed()
ggsave("alignment_comparison.png", dpi = 500)
```
