...
.add_ghost_topics_if_needed = 
  function(aligned_topics, min_k, max_k){
    ...
    gamma_alignment = 
      gamma_alignment %>% mutate(weight = s * weight)
    
    gamma_alignment = 
      bind_rows(gamma_alignment %>% mutate(is_ghost = FALSE),
                gamma_alignment_g) %>% 
...
.compute_leaves_layout = function(
  ...
  leaves_layout_rect = 
    ...
    mutate(m = "words",
           m_num = length(levels(aligned_topics$gamma_alignment$m))+1 + 2,
           x = m_num,
           cum_f = cumsum(f),
...
.compute_layout_reference = function(gamma_alignment, h_m, M, m_ref){
